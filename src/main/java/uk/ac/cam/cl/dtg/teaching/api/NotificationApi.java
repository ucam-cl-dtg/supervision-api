...
public interface NotificationApi {
	...
	public GetNotification getNotification(@QueryParam("offset") int offset,
			...
			// Authentication
			@QueryParam("userId") String userId, @QueryParam("key") String key);
	...
	public static class GetNotification {
		...
		public GetFormError getFormErrors() {
			return formErrors;
		}

		public void setFormErrors(GetFormError formErrors) {
	...
	public CreateNotification createNotification(
			...
			@FormParam("link") String link, @FormParam("users") String users,
	...
	public static class CreateNotification {
		...
		public CreateFormError getFormErrors() {
			return formErrors;
		}

		public void setFormErrors(CreateFormError formErrors) {
			this.formErrors = formErrors;
		}
	...
	public static class NotificationApiWrapper {
		...
		private static Logger log = LoggerFactory
				.getLogger(NotificationApiWrapper.class);
		 ...
		 * 
		 * NotificationApiWrapper n = new NotificationApiWrapper(dashboardUrl,
		 * apiKey); GetNotification data = n.getNotifications(0, 10,
		 * "dashboard", "userCrsid1"); if (data == null) {
		 * log.error("Internal server error: could not get notifications") }
		 * else { // Manipulate data }
		 */

		public GetNotification getNotifications(int offset, int limit,
				String section, String userId) {
			return getNotificationsWithForeignId(offset, limit, section,
					userId, "");
		}

		public GetNotification getNotificationsWithForeignId(int offset,
				int limit, String section, String userId, String foreignId) {

			try {
				try {
					ClientRequestFactory c = new ClientRequestFactory(
							UriBuilder.fromUri(dashboardUrl).build());
					GetNotification gn = c.createProxy(NotificationApi.class)
							.getNotification(offset, limit, section, foreignId,
									userId, apiKey);

					if (gn == null) {
						log.error("Internal server error: could not get notifications");
						throw new NotificationException(
								"Internal server error: could not get notifications");
						...
						String errors = gn.getFormErrors().getForeignId()
								.toString()
		 ...
		 * 
		 * NotificationApiWrapper n = new NotificationApiWrapper(dashboardUrl,
		 * apiKey); try { n.createNotification("Test message", "dashboard",
		 * "example.com", "userCrsid1,userCrsid2"));
		 * log.info("Successfully created notification"); } catch
		 * (NotificationException e) { log.error(e.getMessage()); }
		 */

		public void createNotification(String message, String section,
				String link, String users) throws NotificationException {
			createNotificationWithForeignId(message, section, link, users, null);
		}

		public void createNotificationWithForeignId(String message,
				String section, String link, String users, String foreignId)
				throws NotificationException {
			try {
				ClientRequestFactory c = new ClientRequestFactory(UriBuilder
						.fromUri(dashboardUrl).build());
				CreateNotification cn = c.createProxy(NotificationApi.class)
						.createNotification(message, section, link, users,
								foreignId, apiKey);
